...
# Show that CpGs that correlate with CSF AD biomarkers are also more reliable than other CpGs 
...
ggboxplot(
  ...
  y = "modified_ICC",
  ylab = "modified ICC",
  xlab = "probes with significant association to CSF AD biomarkers",
  fill = "group_sig",
  palette = "jco",
  #outlier.shape = NA,
) + guides(fill = "none") +
...
ggsave(filename = file.path(dir.plot, "ADNI_modified_ICC_with_sig_CSF_cpg_with_sig_CSF_cpgs_boxplots.pdf"),
       width = 10, height = 6)
```

```{r}
summ.table(reliable_df, Group = "group_sig", type = "modified_ICC") %>%
  write_csv(file.path(dir.reliable.score, "ADNI_modified_ICC_vs_CSF_groups.csv"))
```
...
```{r}
reliable_df$group_sig_tau <- ifelse(reliable_df$cpg %in% 
                                      (single.sig.cpgs %>% 
                                         filter(CSF == "PTAU") %>%
                                         pull(cpg)), "groupA", "groupB")
```

### Boxplots group A vs B
...
```{r}
reliable_df.ad <- inner_join(reliable_df, file.list.lm$ADNI_AD_PTAU %>% select(cpg, Estimate.bacon))
reliable_df.cn <- inner_join(reliable_df, file.list.lm$ADNI_CN_PTAU %>% select(cpg, Estimate.bacon))
```
...
```{r}
reliable_df$group_sig_abeta <- ifelse(reliable_df$cpg %in% 
                                      (single.sig.cpgs %>% 
                                         filter(CSF == "ABETA") %>%
                                         pull(cpg)), "groupA", "groupB")
```
...
```{r}
reliable_df.ad <- inner_join(reliable_df, file.list.lm$ADNI_AD_ABETA %>% select(cpg, Estimate.bacon))
reliable_df.cn <- inner_join(reliable_df, file.list.lm$ADNI_CN_ABETA %>% select(cpg, Estimate.bacon))
```
